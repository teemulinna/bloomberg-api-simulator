...
export class BloombergSimulator extends EventEmitter {
  ...
  constructor(config: GeneratorConfig = {}) {
    ...
    // Initialize @ruvector/agentic-synth if API keys are configured
    if (process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY) {
      try {
        ...
        this.emit('log', '✅ @ruvector/agentic-synth integration enabled (NPX mode)');
      } catch (error) {
    ...
    // Initialize Azure OpenAI as fallback if configured
    if (AzureOpenAIClient.isConfigured()) {
      try {
        this.azureClient = new AzureOpenAIClient();
        this.useAzureAI = true;
        this.emit('log', '✅ Azure OpenAI fallback enabled');
      } catch (error) {
  ...
  private async generateNews(symbols: string[]): Promise<News> {
    // Try @ruvector/agentic-synth FIRST (primary AI provider)
    if (this.useAgenticSynth && this.agenticSynth) {
      try {
        ...
        this.emit('log', `⚠️  AgenticSynth failed, trying Azure fallback: ${error}`);
      }
    }

    // Fallback to Azure OpenAI if agentic-synth fails
    if (this.useAzureAI && this.azureClient) {
      try {
        ...
        this.emit('log', `⚠️  Azure OpenAI failed, using mock data: ${error}`);
      }
    }
