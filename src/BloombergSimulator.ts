...
export class BloombergSimulator extends EventEmitter {
  ...
  private agenticSynth: AgenticSynthWrapper | null = null;
  private useAgenticSynth: boolean = false;
  ...
  constructor(config: GeneratorConfig = {}) {
    ...
    // Initialize Azure OpenAI as PRIMARY provider if configured
    if (AzureOpenAIClient.isConfigured()) {
      try {
        this.azureClient = new AzureOpenAIClient();
        this.useAzureAI = true;
        this.emit('log', '✅ Azure OpenAI (Primary AI provider) enabled');
      } catch (error) {
    ...
    // Initialize @ruvector/agentic-synth as SECONDARY provider if API keys configured
    if (process.env.GEMINI_API_KEY || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY) {
      try {
        // Priority: Gemini (secondary) > OpenAI > Anthropic (tertiary)
        ...
        this.agenticSynth = new AgenticSynthWrapper({ provider, apiKey });
        this.useAgenticSynth = true;
        this.emit('log', `✅ @ruvector/agentic-synth (${provider}) as secondary provider`);
      } catch (error) {
        this.emit('log', `⚠️  AgenticSynth configuration error: ${error}`);
        this.useAgenticSynth = false;
      }
    }
  ...
  /**
   * Generate news with sentiment using AI (agentic-synth primary, Azure fallback)
   */
  private async generateNews(symbols: string[]): Promise<News> {
    // Try Azure OpenAI FIRST (primary AI provider)
    if (this.useAzureAI && this.azureClient) {
      try {
        ...
        this.emit('log', `⚠️  Azure OpenAI failed, trying agentic-synth fallback: ${error}`);
      }
    }

    // Fallback to @ruvector/agentic-synth (Gemini > OpenAI > Anthropic)
    if (this.useAgenticSynth && this.agenticSynth) {
      try {
        const synthNews = await this.agenticSynth.generateNews(symbols, 1);
        if (synthNews && synthNews.length > 0) {
          const newsItem = synthNews[0];
          const sentimentScore =
            newsItem.sentiment === 'bullish' ? 0.7 :
            newsItem.sentiment === 'bearish' ? -0.7 : 0;
          ...
          return {
            ...
            timestamp: new Date(),
            headline: newsItem.headline,
            summary: `Generated by @ruvector/agentic-synth for ${symbols.join(', ')}`,
            symbols,
            sentiment: newsItem.sentiment as 'bullish' | 'bearish' | 'neutral',
            sentimentScore,
            impact: newsItem.impact as 'low' | 'medium' | 'high' || 'medium',
            categories: ['earnings', 'market-update'],
            source: 'Bloomberg Terminal (AgenticSynth AI)'
          };
        }
      } catch (error) {
        this.emit('log', `⚠️  AgenticSynth failed, using mock data: ${error}`);
      }
    }
